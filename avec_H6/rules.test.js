...
const heuristics = globalThis.CantStopHeuristics;
...
{
  assert.equal(heuristics.rule28.name, "H4 Rule of 28");
  assert.equal(heuristics.h5.name, "H5 Constant Weights");
  assert.equal(heuristics.h6.name, "H6 DP-3 min turns");
  assert.equal(Math.round(heuristics.h6.benchmarkMean * 10) / 10, 17.9);
}

(() => {
  ...
  const CW_MOVE_WEIGHTS = { 2: 7, 3: 0, 4: 2, 5: 0, 6: 4, 7: 3, 8: 4, 9: 0, 10: 2, 11: 0, 12: 7 };
  const CW_PROGRESS_WEIGHTS = { 2: 7, 3: 7, 4: 3, 5: 2, 6: 2, 7: 1, 8: 2, 9: 2, 10: 3, 11: 7, 12: 7 };
  const CW_MARKER_PENALTY = 6;
  const CW_STOP_THRESHOLD = 29;

  function chooseActionConstantWeights(progress, tempProgress, legalActions) {
    return maxBy(legalActions, (action) =>
      action.reduce((value, column) => {
        const opensNew = Object.hasOwn(tempProgress, column) ? 0 : 1;
        return value + (CW_MOVE_WEIGHTS[column] || 0) - CW_MARKER_PENALTY * opensNew;
      }, 0),
    );
  }

  function shouldStopConstantWeights(tempProgress) {
    const openColumns = listOpenColumns(tempProgress);
    ...
    let value = openColumns.reduce(
      (score, column) => score + (CW_PROGRESS_WEIGHTS[column] || 0) * (tempProgress[column] + 1),
      0,
    );
    ...
    return value >= CW_STOP_THRESHOLD;
  }

  const H6_TARGET_COLUMNS = [2, 7, 12];
  const H6_TARGET_SET = new Set(H6_TARGET_COLUMNS);

  function h6ActionScore(action, progress, tempProgress) {
    const simulatedTemp = cloneMap(tempProgress);
    return action.reduce((score, column) => {
      const isTarget = H6_TARGET_SET.has(column);
      const current = progress[column] + (simulatedTemp[column] || 0);
      const remaining = Math.max(NS[column] - current, 1);
      const opensNew = Object.hasOwn(simulatedTemp, column) ? 0 : 1;
      const finishBonus = remaining <= 1 ? 60 : remaining <= 2 ? 25 : 0;
      const targetBonus = isTarget ? 100 : -100;
      const speedBonus = isTarget ? 20 / remaining : 0;

      simulatedTemp[column] = (simulatedTemp[column] || 0) + 1;
      return score + targetBonus + speedBonus + finishBonus - 3 * opensNew;
    }, 0);
  }

  function chooseActionH6MinTurns(progress, tempProgress, legalActions) {
    const targetActions = legalActions.filter((action) => action.some((column) => H6_TARGET_SET.has(column)));
    const candidates = targetActions.length > 0 ? targetActions : legalActions;
    return maxBy(candidates, (action) => h6ActionScore(action, progress, tempProgress));
  }

  function shouldStopH6MinTurns(progress, tempProgress) {
    const openColumns = listOpenColumns(tempProgress);
    ...
    const targetTemp = openColumns.filter((column) => H6_TARGET_SET.has(column));
    const completedTarget = targetTemp.some((column) => progress[column] + tempProgress[column] >= NS[column]);
    const tempSteps = openColumns.reduce((sum, column) => sum + tempProgress[column], 0);
    const targetSteps = targetTemp.reduce((sum, column) => sum + tempProgress[column], 0);

    if (completedTarget) {
      return true;
    }

    if (targetTemp.length === 3) {
      return targetSteps >= 2;
    }
  ...
  const HEURISTICS = {
    ...
    h5: {
      name: "H5 Constant Weights",
      decide: ({ progress, tempProgress, legalActions }) => {
        const action = chooseActionConstantWeights(progress, tempProgress, legalActions);
        const nextTemp = applyAction(action, progress, tempProgress);
        return { action, stop: shouldStopConstantWeights(nextTemp) };
      },
    },
    h6: {
      name: "H6 DP-3 min turns",
      benchmarkMean: 17.876,
      decide: ({ progress, tempProgress, legalActions }) => {
        const action = chooseActionH6MinTurns(progress, tempProgress, legalActions);
        const nextTemp = applyAction(action, progress, tempProgress);
        return { action, stop: shouldStopH6MinTurns(progress, nextTemp) };
      },
    },
...
class TwoPlayerController {
  ...
  globalThis.CantStopHeuristics = HEURISTICS;
